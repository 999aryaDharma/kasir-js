...
function ProductsDataTable() {
	...
	// State untuk paginasi, dikelola oleh tanstack-table
	const [pagination, setPagination] = React.useState({
		pageIndex: 0,
		pageSize: 10,
	});

	// Debounce input pencarian untuk performa
	const [debouncedGlobalFilter] = useDebounce(globalFilter, 500);

	// 1. Buat URL API yang dinamis dan reaktif terhadap semua filter
	const constructUrl = () => {
	...
	// 3. Ekstrak data produk dan informasi paginasi dari hasil SWR
	...
	const columns = React.useMemo(
		() => [
			...
			{
				id: "categoryName",
				accessorFn: (row) => row.category.name, // Beritahu cara mendapatkan datanya
				header: "Kategori",
			},
			{
				accessorKey: "stock",
	...
	const table = useReactTable({
		data: products, // 4. Gunakan array produk langsung
		columns,
		...
		pageCount: totalPages, // Beri tahu tabel berapa total halaman
		manualFiltering: true, // Beri tahu tabel bahwa filter dikontrol secara manual
		manualPagination: true, // Beri tahu tabel bahwa paginasi dikontrol secara manual
		state: {
			globalFilter,
			columnFilters,
			pagination,
	...
	return (
		// 1. Jadikan Card sebagai flex container vertikal dengan tinggi penuh
		//    Ini akan mengisi ruang yang tersedia di kolom grid.
		...
		<Card className="flex flex-col h-[calc(100vh-3rem)]">
			...
			{/* 2. Buat CardContent mengisi sisa ruang dan bisa di-scroll */}
...
// Komponen untuk Keranjang dan Pembayaran
function TransactionArea() {
	...
	const handlePay = async () => {
		...
		try {
			...
			setLastChange(result.change); // Simpan kembalian dari respons API
			alert(`Transaksi berhasil! Kembalian: ${formatCurrency(result.change)}`);

			// Reset setelah berhasil
			dispatch({ type: "CLEAR_CART" });
			setCash(0);
			setLastChange(null); // Reset kembalian untuk transaksi berikutnya
			mutate("/products"); // Re-fetch data produk untuk update stok
		} catch (error) {
				...
				<div className="space-y-2">
					...
					<Input
						...
						onChange={(e) => {
							...
							setLastChange(null); // Reset kembalian saat input berubah
						}}
						disabled={isLoading}
					/>
